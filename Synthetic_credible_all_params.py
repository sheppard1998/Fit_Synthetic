...
#Add noise to data
def add_noise(trgt_arr, err_arr, num_points):
    add_sub = [-1,1]
    err_weight = np.random.random_sample((15,))
    arr_noise = np.zeros(15)
    for i in range(num_points):
        arr_noise[i] = trgt_arr[i] + random.choice(add_sub)*err_arr[i]
    return arr_noise
...
for k in range(num_iters):
    ...
    #Add noise
    ra_obs_Syn_noise = add_noise(ra_obs_Syn, ra_errs_Syn, len(ra_obs_Syn))
    dec_obs_Syn_noise = add_noise(dec_obs_Syn, dec_errs_Syn, len(dec_obs_Syn))

    overall_start_time = Time.time()

    # Distance to the system in pc:
    d_pc = 2.6314  # cf. Fantino & Casotto
